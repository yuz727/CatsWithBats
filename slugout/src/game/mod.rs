...
use self::components::{Background, Bat, Colliding, Object, Player, Rug};
    ...
    fn build(&self, app: &mut App) {
        ...
        if unsafe { MAP == 1 } {
            app.add_systems(OnEnter(MultiplayerState::Game), setup_mult)
                ...
                .add_systems(
                    Update,
                    player_movement::update_other_players
                        .after(setup_mult)
...
pub fn end_game_mult(
    ...
    for player in client_list_vec.0.iter() {
        if player
            .username
            .contains(player_number.0.to_string().as_str())
            && player.player_info.as_ref().unwrap().health == 0
        {
            ...
            for (player_number, other_player_entity) in other_players.iter() {
                if player
                    .username
                    .contains(player_number.number.to_string().as_str())
                {
