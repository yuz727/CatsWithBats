...
pub fn load_credit_texture(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    for slides in credits_lib {
        commands
            ...
            .insert(Credits)
            .insert(CreditsTimer(Timer::from_seconds(3., TimerMode::Repeating)));
        // Put the next sprite even lower on the z coords
        starting_location -= 10.;
    }
}

pub fn cycle_credits(
    mut commands: Commands,
    mut move_credits: Query<(&mut Transform, &mut CreditsTimer, Entity), With<Credits>>,
    time: Res<Time>,
) {
